...
static uint32_t exposure_percent = DEFAULT_EXPOSURE_PERCENT;
...
static lv_obj_t *camera_settings_items[6];  // Increased to 6 to include exposure + back button
static int current_camera_settings_item = 0;

/**
 * @brief Convert exposure percent (0-100) to microseconds
 * Maps: 0% = 500us (1/2000s), 50% = 2250us (1/444s), 100% = 4000us (1/250s)
 */
static uint32_t exposure_percent_to_us(uint32_t percent)
{
    // Linear mapping: 500us + (percent * 3500 / 100)
    return 500 + (percent * 35);
}
...
static void init_camera_settings_display(void) {
    ...
    lv_slider_set_value(ui_SliderPanelPanelSettingsExposure, exposure_percent, LV_ANIM_OFF);
}
...
static void save_camera_settings(void)
{
    app_storage_save_camera_settings(contrast_percent, saturation_percent, 
                                    brightness_percent, hue_percent, exposure_percent);
    ...
    // Convert exposure percent to microseconds and set
    uint32_t exposure_us = exposure_percent_to_us(exposure_percent);
    app_isp_set_exposure(exposure_us);
}
...
static void switch_to_camera_settings_panel(void) {
    ...
    camera_settings_items[4] = ui_PanelPanelSettingsExposure;
    camera_settings_items[5] = ui_PanelSettingsMenu; // "back to menu" item
...
void ui_extra_btn_down(void)
{
    ...
    switch(current_page) {
        ...
        case UI_PAGE_SETTINGS:
            if (is_camera_settings_panel_active) {
                // camera settings panel is active
                if(current_camera_settings_item < 5) {
                    update_camera_settings_focus(current_camera_settings_item + 1);
                }
            } else {
                // main settings panel is active
...
void ui_extra_btn_right(void)
{
    switch(current_page) {
        case UI_PAGE_SETTINGS:
            if (is_camera_settings_panel_active && current_camera_settings_item < 5) {
                ...
                switch(current_camera_settings_item) {
                    ...
                    case 3: // hue
                        ...
                        break;
                    case 4: // exposure
                        slider = ui_SliderPanelPanelSettingsExposure;
                        value_ptr = &exposure_percent;
                        break;
                }
                ...
                if(slider && value_ptr) {
                    ...
                    switch(current_camera_settings_item) {
                        ...
                        case 4: {
                            uint32_t exposure_us = exposure_percent_to_us(*value_ptr);
                            app_isp_set_exposure(exposure_us);
                            break;
                        }
                    }
...
void ui_extra_btn_menu(void)
{
    ...
    switch(current_page) {
        ...
        case UI_PAGE_SETTINGS:
            if(is_camera_settings_panel_active && current_camera_settings_item == 5 && camera_settings_items[current_camera_settings_item] == ui_PanelSettingsMenu) {
...
void ui_extra_init(void)
{
    ...
    loaded_exposure = DEFAULT_EXPOSURE_PERCENT;
    ...
    err = app_storage_load_camera_settings(&loaded_contrast, &loaded_saturation, 
                                          &loaded_brightness, &loaded_hue, &loaded_exposure);
    if (err == ESP_OK) {
        ...
        exposure_percent = loaded_exposure;
