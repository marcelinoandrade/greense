...
#define NVS_KEY_EXPOSURE "exposure"
...
esp_err_t app_storage_save_camera_settings(uint32_t contrast, uint32_t saturation, 
                                          uint32_t brightness, uint32_t hue, uint32_t exposure_percent)
{
    nvs_handle_t nvs_handle;
    esp_err_t err;
    ...
    // save exposure (as percent 0-100, mapped to 500us-4000us)
    err = nvs_set_u32(nvs_handle, NVS_KEY_EXPOSURE, exposure_percent);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Error saving exposure: %s", esp_err_to_name(err));
        nvs_close(nvs_handle);
        return err;
    }
                                          ...
                                          uint32_t *brightness, uint32_t *hue, uint32_t *exposure_percent)
{
    ...
    // load exposure
    if (exposure_percent) {
        err = nvs_get_u32(nvs_handle, NVS_KEY_EXPOSURE, exposure_percent);
        if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
            ESP_LOGE(TAG, "Error loading exposure: %s", esp_err_to_name(err));
        }
    }
