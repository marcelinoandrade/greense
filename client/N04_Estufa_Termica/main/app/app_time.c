...
bool app_time_init(void) {
    ...
    // Configurar servidores NTP (Brasil)
...
time_t app_time_get_next_acquisition_time(const schedule_time_t schedule[], int schedule_size) {
    ...
    for (int i = 0; i < schedule_size; i++) {
        ...
        next_time.tm_hour = schedule[i].hour;
        next_time.tm_min = schedule[i].minute;
        next_time.tm_sec = 0;
        ...
        if (next == -1) {
            ESP_LOGE(TAG, "Erro ao calcular horário %02d:%02d", schedule[i].hour, schedule[i].minute);
            continue;
        }
    ...
    memcpy(&next_time, &timeinfo, sizeof(struct tm));
    next_time.tm_hour = schedule[0].hour;
    next_time.tm_min = schedule[0].minute;
    next_time.tm_sec = 0;
    next_time.tm_mday++;
    next_time.tm_isdst = -1;
